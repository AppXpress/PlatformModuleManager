      ...
      if (hasElements(criticalFieldsQueryResult) || hasElements(criticalFieldsQueryResult.result)) {
         ...
         if (hasValue(activeOrder) && hasValue(criticalFieldResult)) {
            ...
            for (lineItemCtr; lineItemCtr < lineItems.length; lineItemCtr++) {
               ...
               if (hasValue(lineItem) && hasValue(lineItem.baseItem.reference) && hasValue(lineItem.itemKey)) {
                  ...
                  if (hasValue(lineItem.baseItem) && hasValue(lineItem.baseItem.reference)) {
                     if (lineDifferenceText != lineItem.baseItem.reference[reportName]) {
                        shouldSave = true;
                        lineItem.baseItem.reference[reportName] = truncate(lineDifferenceText, MAX_CUSTOM_REF_LENGTH, MAX_CUSTOM_REF_ERROR);
                     }
                  }
                  ...
                  for (sublineItemCtr; sublineItemCtr < sublineItems.length; sublineItemCtr++) {
                     ...
                     if (hasValue(sublineItem) && hasValue(sublineItem.baseItem) && !ITEM_TYPE_CODE_COMPONENT.equals(sublineItem.baseItem.itemTypeCode)) {
                        ...
                        //Set line level field if needed
                        if (hasValue(sublineItem.baseItem) && hasValue(sublineItem.baseItem.reference)) {
                           if (sublineDifferenceText != sublineItem.baseItem.reference[reportName]) {
                              shouldSave = true;
                              sublineItem.baseItem.reference[reportName] = truncate(sublineDifferenceText, MAX_CUSTOM_REF_LENGTH, MAX_CUSTOM_REF_ERROR);
                           }
                        }
                     }
      ...
      if (hasValue(lineItem) && hasValue(lineItem.baseItem.reference) && hasValue(lineItem.itemKey)) {
         ...
         if (hasValue(lineItem) && hasElements(lineItem.orderItem)) {
            ...
            for (sublineCtr; sublineCtr < sublineItems.length; sublineCtr++) {
               ...
               if (hasValue(sublineItem) && hasValue(sublineItem.itemKey)) {
      ...
      if (hasValue(lineItem) && hasValue(lineItem.baseItem.reference) && hasValue(lineItem.itemKey)) {
         ...
         if (hasValue(lineItem) && hasElements(lineItem.orderItem)) {
            ...
            for (sublineCtr; sublineCtr < sublineItems.length; sublineCtr++) {
               ...
               if (hasValue(sublineItem) && hasValue(sublineItem.baseItem) && hasValue(sublineItem.baseItem.reference) && 
...
function getMatchingLine(knownItem, listOfItems) {
   ...
   if (hasValue(knownItem) && hasElements(listOfItems) && hasValue(knownItem.baseItem.reference) && hasValue(knownItem.itemKey)) {
      ...
      for (matchingLineCtr = 0; matchingLineCtr < listOfItems.length; matchingLineCtr++) {
         ...
         if (hasValue(itemToCompare) && hasValue(itemToCompare.baseItem.reference)) {
