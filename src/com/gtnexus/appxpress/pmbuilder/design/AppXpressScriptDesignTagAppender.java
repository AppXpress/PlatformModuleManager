...
public class AppXpressScriptDesignTagAppender implements
		ScriptDesignTagModifier {
	...
	AppXpressScriptDesignTagAppender(Document xml, File customObject,
			String fileExtension, String mimeTypeValue, String genericFileName) {
		this.xml = xml;
	...
	private void addScriptingTag(Document doc, String customObjectName) {
		...
		if (dom != null && (xmlDesignTag = dom.item(0)) != null) {
			xmlDesignTag
					.appendChild(generateScriptingTag(doc, customObjectName));
		}
	}
	...
	private Node generateScriptingTag(Document doc, String customObjectName) {
		...
		filename.appendChild(doc
				.createTextNode(buildFileName(customObjectName)));
		scripting.appendChild(filename);
		final Element fieldMaskingEnabled = doc
				.createElement(FIELD_MASKING_ENABLED);
	...
	private String buildFileName(String customObjectName) {
		final String fileTypeTemplate = SCRIPT_DESIGN + $ + customObjectName
				+ "%s";
