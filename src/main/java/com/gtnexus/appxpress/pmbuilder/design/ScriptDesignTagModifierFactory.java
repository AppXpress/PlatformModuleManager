...
public class ScriptDesignTagModifierFactory {
	...
	private static final FileDescriptorTriplet zipDescriptor = new FileDescriptorTriplet(
			ZIP_EXTENSION, APPLICATION_ZIP, GENERIC_FILE_ZIP);
	private static final FileDescriptorTriplet jsDescriptor = new FileDescriptorTriplet(
			JS_EXTENSION, TEXT_JS, GENERIC_FILE_JS);

	public ScriptDesignTagModifierFactory() {
	}

	public ScriptDesignTagModifier newTagModifierFor(final Document xml,
			final File customObject, final boolean isJS) {
		if (isJS) {
	...
	private ScriptDesignTagModifier buildModifier(final Document xml,
			final File customObject, final FileDescriptorTriplet desc) {
		if (hasScriptingTag(xml)) {
			return new AppXpressScriptDesignTagModifier(xml, customObject,
					desc.fileExtension, desc.mimeType);
		} else {
			return new AppXpressScriptDesignTagAppender(xml, customObject,
					desc.fileExtension, desc.mimeType, desc.genericFileName);
		}
	}
	...
	private boolean hasScriptingTag(final Document doc) {
		final NodeList scriptingFeature = doc
	...
	private static class FileDescriptorTriplet {
		...
		FileDescriptorTriplet(final String fileExtension,
				final String mimeType, final String genericFileName) {
