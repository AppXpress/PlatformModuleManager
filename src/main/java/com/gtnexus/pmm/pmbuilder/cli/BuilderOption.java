...
public enum BuilderOption implements CLICommandOption {
    ...
    private final static Set<CLICommandOption> allOptions = new ImmutableSet.Builder<CLICommandOption>()
	    .addAll(EnumSet.allOf(BuilderOption.class)).build();

    private BuilderOption(String flag, String name, Class<?> type, String description, boolean hasArg,
	...
	if (this.isMandatory) {
	    throw new UnsupportedOperationException(this.name + " is a mandatory field, and must come from "
